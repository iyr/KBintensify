...
void stateMachine::saveUserSettings(void){
  ...
  FsFile configFile = this->sd->open("/userSettings.cfg", FILE_WRITE);
  if (!configFile) {
...
void stateMachine::loadUserSettings(const char* filepath){
  ...
  FsFile file = this->sd->open(filepath);
...
void stateMachine::updateSM(void){
  if (abs(millis() - this->kspmTimer0) >= 250) {
    uint8_t keySum = 0;

    // Get sum of keystroke inputs, shift array up one
    for(uint8_t i = 5; i > 0; i--){
      keySum += this->kpmBuff[i];
      this->kpmBuff[i] = this->kpmBuff[i-1];
    }

    // Insert latest keystroke count to beginning of array
    keySum += this->kpmBuff[0];
    this->kpmBuff[0] = this->numKeyEvents;
    keySum += this->kpmBuff[0];

    // Get average keystrokes per 6 seconds, extrapolate to one minute
    float meanKS = (((float)keySum / 7.0f) / 1.5f)*60.0f;
    this->kspm = (uint16_t)meanKS;

    // Reset counter vars
    this->numKeyEvents = 0;
    this->kspmTimer0 = millis();
  }
  /*
  float ksps0 = 60000.0f / float(this->kspmTimer-this->kspmTimerLast);
  float ksps1 = 60000.0f / float(millis()-this->kspmTimerLast);
  float ksps2 = (ksps0 + ksps1)/2.0f;
  //this->kspm = min((uint16_t)ksps0, (uint16_t)ksps1);
  this->kspm = min((uint16_t)ksps1, (uint16_t)ksps2);
  //this->kspm = (uint16_t)ksps2;
  this->numKeyEvents = constrain(numKeyEvents--, 0, 255);
  */
  /*
  if (abs(millis() - this->kspmTimer) >= 1000){
    this->kspm = this->numKeyEvents * 60;
    this->numKeyEvents = 0;
    this->kspmTimer = millis();
  }
  */

  if (abs(millis() - this->TouchUpdateTimer) >= this->TouchUpdateRate) {
    this->updateTouchStatus();
    this->TouchUpdateTimer = millis();
  }

  if (abs(millis() - this->repeatActionTimer) >= this->repeatActionDelay) {
    this->repeatAction = true & this->touchPressHeld;
    this->repeatActionTimer = millis();
  } else {
  ...
  if (this->BLrMode & 1) this->BLLevel = 255;
    ...
    if ((this->BLrMode & 32)  && this->currTouch == true)   blt = constrain(blt+32, 0, 1024);

    // Increase BL if BLrMode is set to respond to keystroke input
    if ((this->BLrMode & 64)  && this->numKeysPressed > 0)  blt = constrain(blt+32, 0, 1024); 
    ...
    if ((this->BLrMode & 128) && this->proxlux != NULL) {
      ...
      static uint16_t prox = 0;
      if (abs(millis() - this->BLfuncTimer) >= this->BLfuncRate) {
        prox = this->proxlux->getProximity();
      ...
      if (proxSense && prox > this->BLthresh) {
#ifdef DEBUG
        Serial.print("PROXIMITY DETECTED: ");
        Serial.print(prox);
        Serial.println();
#endif
        blt = constrain(blt+24, 0, 1024);
      }
    }
...
void stateMachine::initDevices(void){
  memset(backBuff, 0, DISP_WIDTH*DISP_HEIGHT*2);
  memset(frameBuff, 0, DISP_WIDTH*DISP_HEIGHT*2);
  memset(wallpaper, 0, DISP_WIDTH*DISP_HEIGHT*2);
  this->tft->begin();
  this->tft->setFrameBuffer(frameBuff);
...
void stateMachine::setScreen(const uint8_t screen) {
  ...
  if (screen == 255) {
    Serial.print("Screen slots: ");
    Serial.print(this->cycleLastScreen[0]);
    Serial.print(", ");
    Serial.print(this->cycleLastScreen[1]);
    Serial.println();
...
// Used for generating repititious inputs for long presses
const bool stateMachine::getRepeatAction(void){
...
const uint16_t stateMachine::getKPM(void){
  return this->kspm;
};

void stateMachine::incKeyEvents(void){
  this->kspmTimerLast = this->kspmTimer;
  this->kspmTimer = millis();
  this->numKeyEvents++;
  return;
};
...
uint16_t*  stateMachine::getWallpaperPtr(void){
  return this->userBackground;
};

void  stateMachine::drawWallpaper(void){
  return this->tft->writeRect(0, 0, DISP_WIDTH, DISP_HEIGHT, this->userBackground);
};
