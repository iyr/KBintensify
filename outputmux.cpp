...
void doOutputMultiplexor(stateMachine* sm){
	...
	// bitmaps of 16 bytes representing enabled outputs
	//static uint8_t 	shortcuts[8][16] = {0};
	uint8_t	shortcutBuff[16] = {0};

	// specify whether a shortcut slot has a shortcut set/saved
	//static bool			savedShortcuts[8] = {0};
  ...
  char titleBuff[32] = {'\0'};
	...
	for (uint8_t i = scOffset; i < 4+scOffset; i++){
  	...
  	shortcutSaved?memset(titleBuff, '\0', 16):strcpy(titleBuff, "Add Shortcut");
  	if ((shortcutSaved?
